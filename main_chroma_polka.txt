...
int main(int argc, char* argv[]) {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        std::vector<glm::vec3> lightDirs = {
            ...
            glm::normalize(glm::vec3(0.5f, 0.5f, -1.0f)),
            glm::normalize(glm::vec3(-0.6f, -0.8f, 0.3f)),
            glm::normalize(glm::vec3(0.3f, -1.0f, -0.5f))
        };
        for (size_t i = 0; i < lightDirs.size(); ++i) {
            std::string uniformName = "lightDirs[" + std::to_string(i) + "]";
            toonShader.setVec3(uniformName, lightDirs[i]);
        }
        toonShader.setInt("numLights", static_cast<int>(lightDirs.size()));
        ...
        x_pos += 0.0005f;
        z_pos += 0.0005f;
        if (x_pos > 2 * 3.14159) x_pos = 0.0f;
        if (z_pos > 2 * 3.14159) z_pos = 0.0f;
        cameraPos = glm::vec3(20 * sin(x_pos), 0.5f, -20 * cos(z_pos));
    }
