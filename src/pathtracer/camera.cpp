...
Ray Camera::generate_ray(double x, double y) const {
  ...
  //
	...
	double sensor_x = 2 * tan(radians(hFov / 2)) * (x - 0.5);
	double sensor_y = 2 * tan(radians(vFov / 2)) * (y - 0.5);
	
	Vector3D sensor_pos = Vector3D(sensor_x, sensor_y, -1);	
	Vector3D dir = c2w * sensor_pos;
	dir /= dir.norm();
